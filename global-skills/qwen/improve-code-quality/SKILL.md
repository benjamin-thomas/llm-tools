...
- **Performance issues** — N+1 queries, algorithmic inefficiencies, unnecessary allocations in hot paths
...
- **Explore agent** (`subagent_type: "Explore"`) — quick surface-level sweep. Fast scanning for obvious issues: naming problems, duplicated blocks, deeply nested conditionals, long methods, obvious performance anti-patterns, security red flags.
- **General-purpose agent** (`subagent_type: "general-purpose"`) — deeper analysis. Complex control flow, misleading names, context-dependent performance issues (N+1 across call chains), convention violations, duplication across files.
...
Dispatch sub-agents in parallel. Budget: **max 10 sub-agents per dispatch phase**. Recommended split: roughly 2/3 Explore, 1/3 general-purpose, adapted to the volume of code:
- Small scope (1-5 files): 2 Explore + 1 general-purpose
- Medium scope (6-15 files): 4 Explore + 2 general-purpose
- Large scope (16+ files): 7 Explore + 3 general-purpose
...
#### Explore sub-agents — surface-level sweep

Each Explore agent receives a batch of files and looks for obvious issues:
...
#### General-purpose sub-agents — deeper analysis

Each general-purpose agent reviews files for subtler issues:
...
- **Explore findings** → dispatch a general-purpose sub-agent to confirm or dismiss. Provide the specific findings and the relevant code. The general-purpose agent should assess whether the finding represents a real quality issue worth fixing.
- **General-purpose findings** → dispatch a general-purpose sub-agent to verify. Provide the findings and relevant code. Focus on whether the suggested improvements are genuinely better or just different.
- **If both Explore and general-purpose found nothing** → dispatch a single general-purpose sub-agent with the code under review. Tell it: "Explore and general-purpose both reviewed this code and found no issues. Perform a final review focusing on: subtle complexity, misleading abstractions, and patterns that may cause problems as the codebase evolves. Do not re-check obvious issues. Only flag things you are highly confident about."
