...
**Does NOT belong**: coding patterns, style preferences, framework conventions, file structure details. These are observable from the code or belong in project config files.
...
For each subject, explore the codebase freely. Use whatever tools and strategies make sense — read code, trace call paths, check git history, search for patterns, use sub-agents for parallel exploration. There is no prescribed exploration method; adapt to the subject and codebase.
